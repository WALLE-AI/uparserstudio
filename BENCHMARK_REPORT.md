...
> **2026-09-11/14 新协议 navidc-ocr 接入双榜单并对齐上游后处理**（Part A §8、Part B §4）：
> opendataloader-bench Overall `0.9089`（榜单第 5/44）；OmniDocBench 在把公式/文本后处理
> 按上游 `post_process/` 逐函数对齐后，**Formula CDM `0.9631` 对上游宣称的 `0.9636`，
> 差 0.0005 实质追平**，Formula Edit `0.1009 → 0.0636`（−37%）。
...
| 结论一句话(最新，2026-09-11 §8/§4) | mineru-vlm 0.9252 >**navidc-ocr 0.9089** > pipeline 0.9086 > native 0.8766；navidc-ocr 的 TEDS 0.9618 为全榜第二档 | mineru-vlm 91.3490 / pipeline 88.3451；**navidc-ocr 对齐后 Formula CDM 0.9631（≈上游 0.9636）**，Text Edit 0.0593 仍落后上游宣称的 0.027 | 两路径均 544/544 成功；当前配置下 VLM 墙钟快 4.314×，但并发不对称且服务端`>=1000` 门槛未通过 |
...
- 原始 Markdown builder 具有语言感知空格、英文行末断词合并、 dMarkdown 转义和标题层级恢复；uparser
...
| 指标                     |   首轮 | +header/footer | **+采样+完整后处理对齐** |  上游模型卡宣称 |
| ------------------------ | -----: | -------------: | -----------------------------: | --------------: |
| **Formula CDM↑**  | 不可用 |         0.9419 |               **0.9631** |          0.9636 |
| **Formula Edit↓** | 0.0981 |         0.1009 |               **0.0636** |              — |
| Text Edit↓              | 0.0603 |         0.0626 |               **0.0593** | **0.027** |
| Order Edit↓             | 0.1381 |         0.1405 |               **0.1356** |           0.122 |
| Table TEDS↑             | 0.9644 |         0.9585 |                         0.9578 |          0.9705 |
| TEDS-S↑                 | 0.9707 |         0.9645 |                         0.9641 |          0.9852 |

**Formula CDM `0.9631` vs 上游宣称 `0.9636`,差 0.0005 —— 实质追平**;Formula Edit
同步由 `0.1009` 降到 `0.0636`(−37%)。这一项直接验证了"公式后处理必须按协议对齐"
的判断:此前 navidc 走的是 **MinerU 的** `formula_repair::DEFAULT_CHAIN`,与上游
`post_process/_process_equation` 在定界符(`\[…\]` vs `$$…$$`)、未匹配括号处理
(补全 vs **删除**)、Unicode→LaTeX、`%` 转义四处都不一致。

第三轮同时含采样参数补齐(见 §4.6),两者无法在本轮拆开归因。

Text Edit 仍落后上游(`0.0593` vs `0.027`)。逐页分析见 §4.7。

### 4.5 各轮的具体改动

| 轮次           | 改动                                                           |
| -------------- | -------------------------------------------------------------- |
| 首轮           | 初版适配器;CDM 因未设`CDM_TEXLIVE_ROOT` 而作废               |
| +header/footer | `ascend::ParatextPolicy` 按协议分派(Part A §8.2)            |
| +对齐          | 补齐上游采样参数;新建`navidc_post.rs` 移植 `post_process/` |

### 4.6 采样参数:漏发上游全部抗重复机制

适配器最初只发 `temperature`/`top_p`/`max_tokens`,漏掉上游
`NaviOCRSamplingParams` 的 `top_k=1`、`repetition_penalty=1.0`、
`presence_penalty`、`frequency_penalty`(text/formula/code 用 `0.05`,
table/char/seal 用 `0.005`)、`no_repeat_ngram_size=100`、
`skip_special_tokens=false`。

后果可量化:最差页 `newspaper_2a6b4fa0…` 上,一个块退化成 **9140 字符、
`"because 1-3."` 重复 567 次**(唯一 8-gram 占比 7.1%)。补齐参数后同页
输出 15742→8975 字符、最大段 9140→2524、唯一 8-gram 占比 →1.000、耗时 48s→6s。

全语料退化页 15/1651,单独修复约值 −0.0023 Text Edit——**不足以解释到上游的差距**,
不夸大。

### 4.7 剩余 Text Edit 差距的逐页归因

分布高度偏斜:中位数 `0.0077`、592 页完全正确,但 156 页(edit≥0.2)贡献
**57.6%** 的总误差。在 0.2–0.5 这个最大桶里,**96/133 页的预测文本量与 GT 相当**
(比值 0.8–1.3)——不是多字或漏字的结构问题,而是识别内容/顺序层面的差异。
换言之,渲染/后处理层面能做的已接近上限;上游还有一个**文档去弯曲(dewarping)**
预处理是本适配器明确未实现的。
