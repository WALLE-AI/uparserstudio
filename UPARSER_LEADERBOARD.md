...
> 数据截止：2026-09-20（monkeyocr-v2 两行于本次适配器对齐后全量重测，其余行沿用 2026-09-14）· 本文档只呈现**评测口径与对比数据**，不含归因分析与优化过程。
...
| `monkeyocr-v2` | 0.8827 | 0.8932 | 0.9522 | 0.8326 | 3.011 |
...
| `monkeyocr-v2` | 0.0834 | 0.1990 | 0.8981 | 0.8373 | 0.8745 | 0.1549 |
...
> **MonkeyOCRv2-B-Parsing 的官方值无法并入上表**：其 README 公布的是 OmniDocBench v1.6
> 端到端**综合分 83.3（0–100，越高越好，该榜第一）**，与本表的 edit／TEDS 口径不同，
> 不可换算、也不可直接比较。列出它是为了说明本地 `monkeyocr-v2` 行跑的就是这份权重
> （`127.0.0.1:8011`，`MonkeyOCRv2-B-Parsing`），因此该行的高低反映的是**适配器实现**，
> 而非模型能力。
