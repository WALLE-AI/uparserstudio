...
> 评测日期：2026-08-21；官方 evaluator 复核：2026-08-24
...
6. **完整 OmniDocBench 已通过当前 V2 模型链实跑，但质量未超过历史最佳**：1,651/1,651 页生成完成，
   非零返回码为 0。2026-08-24 用当前 evaluator 复核后，文本/公式 Edit distance 为
   `0.069960`/`0.102585`，表格 TEDS/结构 TEDS 为 `0.906068`/`0.937463`，阅读顺序 Edit distance
   为 `0.136131`。page match 有 1 页使用 timeout fallback；665 个表格 TEDS 无超时、错误或异常。
...
|---|---:|---:|---:|---:|---:|
| **当前 V2（2026-08-24 evaluator 复核）** | **0.069960** | **0.102585** | **0.906068** | **0.937463** | **0.136131** |
...
生成输出为 1,651 个 Markdown，非零返回码 0、空文件 0；其中 1 个文件仅含换行。2026-08-24 复核中，
官方 page match 的 1,651 页有 1 页 timeout fallback，TEDS 665 个样本无 timeout/error/exception。
生成文件首末写入跨度 `2,024.45s`（约 `1.23 pages/s`）；本次官方页匹配约 `1,367s`，表格 TEDS
约 115s。该墙钟数据包含
...
- 当前 MinerU OmniDocBench：**完整性和稳定性通过，综合精度未提升**；相对历史 `mineru-vlm-v2`
  有增有退，相对 `2605-current`、`surpass` 和 `official` 基线整体仍弱，不能标记为质量领先。
