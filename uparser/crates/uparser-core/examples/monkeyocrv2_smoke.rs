//! Manual smoke test for `MonkeyOcrV2Adapter` against a real vLLM
//! endpoint. Not part of the test suite (network-dependent) — run
//! explicitly:
//!
//!   NO_PROXY=127.0.0.1,localhost cargo run --example monkeyocrv2_smoke -- \
...
async fn main() {
    ...
    let png_path = args
        .get(1)
        .expect("usage: monkeyocrv2_smoke <page.png> [endpoint] [model]");
    ...
    let model = args
        .get(3)
        .cloned()
        .unwrap_or_else(|| "monkeyocrv2".to_string());
