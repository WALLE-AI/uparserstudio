//! Manual smoke test for `MineruVlmAdapter` against a real vLLM
//! endpoint. Not part of the test suite (network-dependent) — run
//! explicitly:
//!
//!   NO_PROXY=127.0.0.1,localhost cargo run --example mineru_vlm_smoke -- \
...
async fn main() {
    ...
    let png_path = args
        .get(1)
        .expect("usage: mineru_vlm_smoke <page.png> [endpoint] [model]");
    let endpoint_base = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "http://127.0.0.1:19122/files/parser/v1/chat/completions".to_string());
    let model = args
        .get(3)
        .cloned()
        .unwrap_or_else(|| "MinerU2.5-2604-1.2B".to_string());
    ...
    let adapter = MineruVlmAdapter {
        endpoint_base,
        model,
        ..MineruVlmAdapter::default()
    };
    ...
    match adapter.parse_page(&page, &ctx).await {
        Ok(blocks) => {
            ...
            for (i, b) in blocks.iter().enumerate() {
                eprintln!(
                    "--- block {i}: category_raw={} category={:?} bbox_px={:?} error={:?}",
                    b.category_raw, b.category, b.bbox_px, b.error
                );
