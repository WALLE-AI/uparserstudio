...
///
/// Both rejections below are input problems, so both report `EXIT_USAGE`.
/// The resource-limit case used to report `EXIT_DEPENDENCY` purely because
/// the limit was applied later, at execute time, than the format check —
/// `ARCHITECTURE.md` §6.1 reserves code 2 for a genuinely unavailable
/// dependency (missing LibreOffice, unreachable endpoint), which this is
/// not. Since O4.3 the caller's `ParseOptions` reach `analyze`, so every
/// pre-execution rejection is reported the same way.
#[test]
fn structured_failures_map_onto_their_own_exit_codes() {
    ...
    Command::cargo_bin("uparser")
        ...
        .assert()
        .code(1)
        .stderr(predicates::str::contains("max_input_bytes"));
}
